...
(function() {
  ...
  new PureCounter();
    ...
    // Bắt sự kiện keyup cho các trường input
    ...
    function validateName() {
        ...
        if (nameValue.length < 5) {
            message.textContent = 'Tên phải có ít nhất 5 ký tự.';
            return false;
        } else {
            message.textContent = '';
            return true;
    ...
    function validateMessage() {
        ...
        const message = document.querySelector('.descript');
        if (messageValue.length < 5) {
            message.textContent = 'Nội dung phản hồi phải có ít nhất 5 ký tự.';
            return false;
        } else {
            message.textContent = '';
            return true;
    ...
    function handleSubmit(event) {
        ...
        // Kiểm tra nếu không có thông báo lỗi (validate hợp lệ)
        if (
            validateName() === true &&
            validateEmail() === true &&
            ...
            console.log(259782349649);
            // Hiển thị spinner và vô hiệu hóa nút "Gửi phản hồi"
            submitBtn.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Đang gửi...';
            submitBtn.disabled = true;

            // Tạo độ trễ 1 giây (1000ms) để mô phỏng việc gửi thành công
            setTimeout(function () {
